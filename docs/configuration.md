...
| `--read-timeout-seconds` | `30` | Per-connection **idle** timeout (slowloris guard): the timer resets whenever the connection moves bytes in either direction, and the connection is killed only after N consecutive seconds with **zero read/write progress**. Healthy transfers that simply take long — a multi-GiB `aws s3 cp` through the gateway — are unaffected. `0` disables it (not recommended). Also settable via the `S4_READ_TIMEOUT_SECONDS` env var (the CLI flag wins when both are given); on Kubernetes use the chart's `extraArgs` (e.g. `--set 'extraArgs={--read-timeout-seconds=300}'`) or `extraEnv`. Caveat: purely server-side phases with no connection traffic still count as idle — compressing an already-received PUT body at a **high `--zstd-level` (19–22)** can exceed the window; prefer the latency-friendly default level and `s4 recompact` cold data instead (see the [Elasticsearch frozen-tier use case](use-cases/elasticsearch-frozen-tier.md)). **Before this fix (≤ v1.5.0)** this flag was a whole-connection wall-clock cap: any request needing more than 30 s of connection lifetime — including an actively-transferring 2 GiB `aws s3 cp` — was killed mid-transfer (#149) |
