...
## Production 機能 (Phase 1 + 2.0 完了状態、16 commits、58 tests)
...
## 既知の制限事項 / Phase 2.1 計画

| 項目 | 現状 | Phase 2.1 計画 |
|------|------|---------------|
| Range GET 帯域効率 | full read + decompress + slice (機能は動く) | sidecar `<key>.s4index` で必要 frame だけ partial GET |
...
| OpenTelemetry traces | metrics + logs のみ | OTLP exporter で trace span も出力 |
