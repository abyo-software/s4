...
`s4-server` ships ~27 `pub mod` declarations from `crates/s4-server/src/lib.rs` so the `s4` binary (which is a separate crate) + the integration tests + the example binaries can reach the surface they need. Only the four modules with frozen items above (`repair`, `service`, `sse`, `streaming`) are part of the v1.0 contract.

Library consumers MAY `use s4_server::<other_module>::*;` — Rust visibility allows it — but those imports are **not frozen** and may break in any v1.x minor release without notice. The other modules (`audit_log`, `blob`, `cors`, `inventory`, `kms`, `lifecycle`, `mfa`, `multipart_state`, `notifications`, `object_lock`, `policy`, `replication`, `routing`, `select`, `sigv4a`, `tagging`, `tls`, `versioning`, `access_log`, `acme`, `lock_recovery`, `metrics`, `rate_limit`, `state_loader`, `streaming_checksum`, …) exist as `pub mod` for binary-and-tests' needs, not as a published surface.
