...
- `/health` 常に 200 OK (process alive)
- `/ready` backend.list_buckets で接続性確認 → 200 / 503 (4xx 認証エラーは
  「backend は生きている」とみなし 200)
...
| 項目 | 現状 | Phase 2.1 計画 |
|------|------|---------------|
| Range GET on S4 object | `InvalidRange` で reject | frame index を sidecar object に置いて部分解凍 |
| GPU streaming compress | bytes-buffered (5 GB max) | nvCOMP batched API を per-chunk pipeline に |
| Multipart per-part 別 codec | 全 part 共通 codec | frame header に codec ID 4 byte 追加 |
| Multipart 最終 part padding | 全 part 一律 5 MiB pad | server-side copy で trim |
...
| /metrics endpoint (Prometheus) | JSON log で代替 | OpenTelemetry exporter |

## テスト

```bash
# unit + in-process integration tests (43 tests、~1 sec)
cargo test --workspace

# Docker MinIO 経由の E2E (6 tests、~7 sec、要 docker daemon)
cargo test --workspace -- --ignored --test-threads=1

# GPU E2E (要 NVCOMP_HOME + CUDA-capable GPU、追加 4 tests)
export NVCOMP_HOME=/path/to/nvcomp-linux-x86_64-5.x.x_cuda12-archive
...
- `tests/http_e2e.rs`: 実 hyper server を spawn し aws-sdk-s3 client で叩く
  (3 tests、roundtrip + list_objects + health/ready)
