...
fn get_bucket_location_output() {
    {
        let us_west_2 = s3s::dto::BucketLocationConstraint::from_static(
            s3s::dto::BucketLocationConstraint::US_WEST_2,
        );
...
fn get_bucket_notification_configuration_output() {
    ...
    let val =
        deserialize::<s3s::dto::GetBucketNotificationConfigurationOutput>(xml.as_bytes()).unwrap();
    assert_eq!(val, default());
    test_serde(&val);
}
...
<AccessControlPolicy xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  ...
  <AccessControlList>
    ...
    assert_eq!(
        grants[0].permission.as_ref().unwrap().as_str(),
        "FULL_CONTROL"
    );
...
fn test_static_vs_from_static() {
    ...
    assert_eq!(
        static_enabled.as_str().as_ptr(),
        BucketVersioningStatus::ENABLED.as_ptr()
    );
}
