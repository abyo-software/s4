...
impl<W: Write> Serializer<W> {
    ...
    pub fn event(&mut self, event: Event<'_>) -> SerResult {
        self.inner
            .write_event(event)
            .map_err(|err| SerError { inner: err })
    }
    ...
    pub fn content_with_ns<T: SerializeContent + ?Sized>(
        &mut self,
        name: &str,
        xmlns: &str,
        val: &T,
    ...
    pub fn content_with_attrs<T: SerializeContent + ?Sized>(
        &mut self,
        name: &str,
        attrs: &[(&str, &str)],
        val: &T,
    ...
    pub fn timestamp(&mut self, name: &str, val: &Timestamp, fmt: TimestampFormat) -> SerResult {
        fmt_timestamp(val, fmt, |b| {
            self.content(name, str::from_ascii_simd(b).unwrap())
        })
    }
}
