...
mod tests {
    ...
    fn etag_xml_serialization_uses_literal_quotes_not_entities() {
        ...
        assert!(
            !xml.contains("&quot;"),
            "ETag must not use HTML entity encoding; got: {xml}"
        );
    }
    ...
    fn etag_xml_serialization_long_value_uses_literal_quotes() {
        ...
        assert!(
            ...
            "Long ETag must use literal quotes; got: {xml}"
        );
        assert!(
            !xml.contains("&quot;"),
            "Long ETag must not use &quot;; got: {xml}"
        );
    }
    ...
    fn create_session_output_xml_serialization() {
        ...
        assert!(
            xml.contains("CreateSessionResult"),
            "root element must be CreateSessionResult: {xml}"
        );
        assert!(
            xml.contains("<Credentials>"),
            "must contain Credentials element: {xml}"
        );
        assert!(
        ...
        assert!(
            xml.contains("<SecretAccessKey>"),
            "must contain SecretAccessKey: {xml}"
        );
        assert!(
            xml.contains("<SessionToken>"),
            "must contain SessionToken: {xml}"
        );
        assert!(
            xml.contains("<Expiration>"),
            "must contain Expiration: {xml}"
        );
    }
}
