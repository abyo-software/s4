...
mod parser {
    ...
    pub fn parse_authorization(mut input: &str) -> IResult<&str, AuthorizationV4<'_>> {
        ...
        let credential = consume(s, |i| {
            delimited(tag("Credential="), parse_credential, tag(",")).parse(i)
        })?;

        consume(s, multispace0)?;
        let parse_headers = separated_list1(tag(";"), take_till(|c| c == ';' || c == ','));
        let signed_headers = consume(s, |i| {
            delimited(tag("SignedHeaders="), parse_headers, tag(",")).parse(i)
        })?;
    ...
    pub fn parse_credential(mut input: &str) -> IResult<&str, CredentialV4<'_>> {
        ...
        let date = consume(s, |i| {
            verify(until_slash1, |s| verify_date(s).is_ok()).parse(i)
        })?;
...
mod tests {
    ...
    fn special_20230204() {
        ...
        assert_eq!(
            ans.signature,
            "d2ff90c5a29855fd7c56251aa4c02c49a1bc258a8cc9c191ba3cfc037c5dab80"
        );
    }
}
