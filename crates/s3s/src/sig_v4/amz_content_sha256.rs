...
impl<'a> AmzContentSha256<'a> {
    ...
    pub fn parse(header: &'a str) -> Result<Self, ParseAmzContentSha256Error> {
        ...
        match header {
            ...
            "STREAMING-AWS4-HMAC-SHA256-PAYLOAD-TRAILER" => {
                Ok(Self::StreamingAws4HmacSha256PayloadTrailer)
            }
            "STREAMING-AWS4-ECDSA-P256-SHA256-PAYLOAD" => {
                Ok(Self::StreamingAws4EcdsaP256Sha256Payload)
            }
            "STREAMING-AWS4-ECDSA-P256-SHA256-PAYLOAD-TRAILER" => {
                Ok(Self::StreamingAws4EcdsaP256Sha256PayloadTrailer)
            }
...
mod tests {
    ...
    fn parse_streaming_aws4_ecdsa_p256_sha256_payload_trailer() {
        let v =
            AmzContentSha256::parse("STREAMING-AWS4-ECDSA-P256-SHA256-PAYLOAD-TRAILER").unwrap();
        assert_eq!(
            v,
