...
pub fn create_string_to_sign(
    ...
    match mode {
        ...
        Mode::PresignedUrl => {
            let expires = qs
                .and_then(|qs| qs.get_unique("Expires"))
                .unwrap_or_default();
...
mod tests {
    ...
    fn examples() {
...

        {
            ...
            let string_to_sign =
                create_string_to_sign(Mode::HeaderAuth, method, uri_path, qs, &headers, vh_bucket);
    ...
    fn regression_sig_v2_virtual_hosted_bucket() {
        ...
        let headers =
            OrderedHeaders::from_slice_unchecked(&[("date", "Thu, 14 Mar 2024 12:00:00 +0000")]);
        let qs = None;
