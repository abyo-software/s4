...
mod tests {
    ...
    fn test_encode_respects_75_char_limit() {
        ...
        for word in encoded.split(' ') {
            assert!(
                word.len() <= 75,
                "Encoded word exceeds 75 characters: {} (len={})",
                word,
                word.len()
            );
        }
    ...
    fn test_encode_long_string_multiple_words() {
        ...
        assert!(
            word_count > 1,
            "Long string should be split into multiple words"
        );
    ...
    fn test_decode_error_display_input_too_large() {
        ...
        assert_eq!(
            err.to_string(),
            "input string too large (max 1048576 bytes)"
        );
    }
    ...
    fn test_encode_error_display_invalid_utf8_boundary() {
        ...
        assert_eq!(
            err.to_string(),
            "failed to find valid UTF-8 character boundary"
        );
    }
