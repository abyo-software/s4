...
/// # Errors
/// Returns an `Err` if the s3 path is invalid
pub fn parse_path_style_with_validation(
    uri_path: &str,
    ...
    let Some(key) = key else {
        return Ok(S3Path::bucket(bucket));
    };
...
/// # Errors
/// Returns an `Err` if the s3 path is invalid
pub fn parse_virtual_hosted_style(
...
mod tests {
    ...
    fn path_style() {
        ...
        let cases = [
            ...
            (
                "/bucket/dir/object",
                Ok(S3Path::object("bucket", "dir/object")),
            ),
    ...
    fn test_path_style_with_custom_validation() {
        ...
        for bucket_name in invalid_names {
            ...
            assert!(
                result.is_err(),
                "Expected error for bucket name: {bucket_name}"
            );
            ...
            assert!(
                result.is_ok(),
                "Expected success for bucket name: {bucket_name}"
            );
        ...
        let result =
            parse_path_style_with_validation("/valid-bucket/key", &RelaxedNameValidation::new());
        assert!(result.is_ok());
    }
    ...
    fn test_virtual_hosted_style_with_custom_validation() {
        ...
        for bucket_name in invalid_names {
            // Should fail with default validation
            let result = parse_virtual_hosted_style_with_validation(
                Some(bucket_name),
                "/key",
    ...
    fn test_virtual_hosted_style_validation_fallback() {
        ...
        let result2 = parse_virtual_hosted_style_with_validation(
            Some("UPPERCASE"),
            "/key",
            &AwsNameValidation::new(),
        );
