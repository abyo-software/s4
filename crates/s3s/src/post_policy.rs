...
impl PostPolicy {
    ...
    fn has_condition_for_field(&self, name: &str) -> bool {
        self.conditions.iter().any(|c| match c {
            PostPolicyCondition::Eq { field, .. }
            | PostPolicyCondition::StartsWith { field, .. } => field == name,
    ...
    fn get_field_value<'a>(
        ...
        url_bucket: Option<&'a str>,
    ) -> S3Result<Option<Cow<'a, str>>> {
        if field == "bucket" {
...
impl RawCondition {
    ...
    fn parse_array_condition(
        items: &[serde_json::Value],
    ) -> Result<PostPolicyCondition, PostPolicyError> {
        if items.is_empty() {
    ...
    fn parse_object_condition(
        ...
        let (field, value) = map
            .into_iter()
            .next()
            .ok_or(PostPolicyError::InvalidCondition)?;
        Ok(PostPolicyCondition::Eq {
            field: normalize_field_name(&field),
            value,
        })
...
mod tests {
    ...
    fn test_parse_policy_json() {
        ...
        assert_eq!(
            policy.expiration,
            make_timestamp("2030-01-01T00:00:00.000Z")
        );
    ...
    fn test_parse_policy_base64() {
        let json =
            r#"{"expiration":"2030-01-01T00:00:00.000Z","conditions":[["eq","$bucket","test"]]}"#;
    ...
    fn test_normalize_field_name() {
        ...
        assert_eq!(normalize_field_name("bucket"), "bucket");
        assert_eq!(
            normalize_field_name("X-Amz-Meta-Custom"),
            "x-amz-meta-custom"
        );
    }
    ...
    fn test_validate_condition_content_length_range_success() {
        ...
        let condition = PostPolicyCondition::ContentLengthRange {
            min: 100,
            max: 1000,
        };
    ...
    fn test_validate_condition_field_content_type() {
        let multipart =
            create_test_multipart(vec![("Content-Type", "image/jpg")], Some("image/jpeg"));
        let condition = PostPolicyCondition::Eq {
            field: "content-type".to_owned(),
            value: "image/jpeg".to_owned(),
        };
    ...
    fn test_validate_condition_field_content_type_right() {
        let multipart =
            create_test_multipart(vec![("Content-Type", "image/jpeg")], Some("image/jpg"));
        let condition = PostPolicyCondition::Eq {
            field: "content-type".to_owned(),
            value: "image/jpeg".to_owned(),
        };
    ...
    fn test_validate_bucket_condition_from_url_mismatch() {
        ...
        let e = PostPolicy::validate_condition(&condition, &multipart, 0, Some("wrongbucket"))
            .unwrap_err();
        assert!(matches!(e.code(), S3ErrorCode::InvalidPolicyDocument));
    }
    ...
    fn test_validate_bucket_form_field_conflicts_with_url_bucket() {
        ...
        let multipart =
            create_test_multipart(vec![("bucket", "form-bucket"), ("key", "mykey")], None);
        let condition = PostPolicyCondition::Eq {
            field: "bucket".to_owned(),
        ...
        let e = PostPolicy::validate_condition(&condition, &multipart, 0, Some("url-bucket"))
            .unwrap_err();
        assert!(matches!(e.code(), S3ErrorCode::InvalidPolicyDocument));
    }
    ...
    fn assert_json_error(json_samples: &[&str]) {
        for json in json_samples {
            let error = PostPolicy::from_json(json).unwrap_err();
            assert!(
                matches!(error, PostPolicyError::Json(_)),
                "Expected Json error for: {json}"
            );
        }
    }
    ...
    fn test_form_field_not_in_policy_is_rejected() {
        ...
        let err = policy
            .validate_conditions_only(&multipart, 0, Some("mybucket"))
            .unwrap_err();
        assert!(matches!(err.code(), S3ErrorCode::AccessDenied));
        assert!(
            err.message()
                .unwrap_or("")
                .contains("success_action_status"),
    ...
    fn test_exempt_fields_not_in_policy_are_allowed() {
        ...
        assert!(
            result_v2.is_ok(),
            "SigV2 exempt fields should not be rejected"
        );
    }
    ...
    fn test_starts_with_condition_covers_field() {
        ...
        assert!(
            result.is_ok(),
            "field covered by starts-with should be accepted"
        );
    }
