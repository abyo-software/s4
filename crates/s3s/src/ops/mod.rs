...
pub struct CallContext<'a> {
    ...
    /// v0.13.0-s4.1 NoVerify mode: see
...
fn extract_qs(req_uri: &Uri) -> S3Result<Option<OrderedQs>> {
    let Some(query) = req_uri.query() else {
        return Ok(None);
    };
    match OrderedQs::parse(query) {
        Ok(ans) => Ok(Some(ans)),
        Err(source) => Err(S3Error::with_source(
...
fn extract_headers(headers: &HeaderMap) -> S3Result<OrderedHeaders<'_>> {
    OrderedHeaders::from_headers(headers)
        .map_err(|source| invalid_request!(source, "invalid headers"))
}
...
fn extract_decoded_content_length(hs: &'_ OrderedHeaders<'_>) -> S3Result<Option<usize>> {
    ...
    match atoi::atoi::<usize>(val.as_bytes()) {
        ...
        None => Err(invalid_request!(
            "invalid header: x-amz-decoded-content-length"
        )),
    }
}
...
async fn prepare(req: &mut Request, ccx: &CallContext<'_>) -> S3Result<Prepare> {
    ...
    let mut content_length;
    {
            ...
            let result = 'parse: {
                    ...
                    && !is_socket_addr_or_ip_addr(host_header)
                {
                    debug!(
                        ?host_header,
                        ?decoded_uri_path,
                        "parsing virtual-hosted-style request"
                    );
        ...
        let transformed_body;
        {
            let mut scx = SignatureContext {
                ...
                skip_verification: ccx.skip_signature_verification,
            };
            ...
            match s3_path {
                ...
                S3Path::Bucket { bucket } => {
                    ...
                    let policy = if let Some(policy_b64) = multipart.find_field_value("policy") {
                        let policy = PostPolicy::from_base64(policy_b64).map_err(|e| {
                            s3_error!(e, InvalidPolicyDocument, "failed to parse POST policy")
                        })?;
                        ...
                        let expiration_time: time::OffsetDateTime =
                            policy.expiration.clone().into();
        ...
        && qs.has("events")
    {
        return Err(s3_error!(
            NotImplemented,
            "listenBucketNotification only works on MinIO"
        ));
    }
