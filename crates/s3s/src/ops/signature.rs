...
pub struct SignatureContext<'a> {
    ...
    /// v0.13.0-s4.1 NoVerify mode: when true, the final HMAC-compare against
    /// the client-supplied signature is skipped (and any per-chunk signature
    /// compares are skipped in the streaming-body decoder, when supported).
    /// The auth provider's `get_secret_key` is still called so a fallback
    /// (e.g. [`AcceptAnyAuth`]) can record the access key; the canonical
    /// request and string-to-sign are still built so streaming bodies are
    /// decoded properly. See
    /// [`S3ServiceBuilder::set_skip_signature_verification`].
    ///
    /// [`AcceptAnyAuth`]: crate::auth::AcceptAnyAuth
...
fn require_auth(auth: Option<&dyn S3Auth>) -> S3Result<&dyn S3Auth> {
    auth.ok_or_else(|| {
        s3_error!(
            NotImplemented,
            "This service has no authentication provider"
        )
    })
}
...
impl SignatureContext<'_> {
        ...
        let multipart = {
            let Some(mime) = self.mime.as_ref() else {
                return Err(invalid_request!(
                    "internal error: mime was unexpectedly None"
                ));
            };
    ...
    pub async fn v4_check_post_signature(
        &mut self,
        multipart: Multipart,
    ) -> S3Result<CredentialsExt> {
        let auth = require_auth(self.auth)?;

        let info = PostSignatureV4::extract(&multipart)
            .ok_or_else(|| invalid_request!("missing required multipart fields"))?;
        ...
        let credential = CredentialV4::parse(info.x_amz_credential)
            .map_err(|_| invalid_request!("invalid field: x-amz-credential"))?;

        let amz_date = AmzDate::parse(info.x_amz_date)
            .map_err(|_| invalid_request!("invalid field: x-amz-date"))?;
        ...
        if signature != expected_signature {
            if self.skip_verification {
                debug!(?signature, expected=?expected_signature, "signature mismatch (NoVerify mode — accepted)");
            } else {
    ...
    pub async fn v4_check_presigned_url(&mut self) -> S3Result<CredentialsExt> {
        ...
        let presigned_url = PresignedUrlV4::parse(qs)
            .map_err(|err| invalid_request!(err, "missing presigned url v4 fields"))?;
...

        {
            ...
            let config = self.config.snapshot();
            let max_skew_time =
                time::Duration::seconds(i64::from(config.presigned_url_max_skew_time_secs));
            if duration.is_negative() && duration.abs() > max_skew_time {
                return Err(s3_error!(
                    RequestTimeTooSkewed,
                    "request date is later than server time too much"
                ));
            }
        ...
        let signature = {
            let headers =
                self.hs
                    .find_multiple_with_on_missing(&presigned_url.signed_headers, |name| {
            ...
            let canonical_request =
                sig_v4::create_presigned_canonical_request(method, uri_path, qs.as_ref(), &headers);

            let amz_date = &presigned_url.amz_date;
            let string_to_sign =
                sig_v4::create_string_to_sign(&canonical_request, amz_date, region, service);
    ...
    pub async fn v4_check_header_auth(&mut self) -> S3Result<CredentialsExt> {
        ...
        let amz_date = extract_amz_date(&self.hs)?
        ...
        let signature = {
            ...
            let headers =
                self.hs
                    .find_multiple_with_on_missing(&authorization.signed_headers, |name| {
                        // HTTP/2 replaces `host` header with `:authority`
                        // but `:authority` is not in the request headers
                        // so we need to add it back if `host` is in the signed headers
                        if name == "host"
            ...
            let canonical_request = match amz_content_sha256 {
                ...
                Some(AmzContentSha256::StreamingAws4HmacSha256PayloadTrailer) => {
                    sig_v4::create_canonical_request(
                        method,
                        uri_path,
                        query_strings,
                ...
                Some(AmzContentSha256::UnsignedPayload) => sig_v4::create_canonical_request(
                    method,
                    uri_path,
                    query_strings,
                    &headers,
                ...
                Some(AmzContentSha256::StreamingUnsignedPayloadTrailer) => {
                    sig_v4::create_canonical_request(
                        method,
                        uri_path,
                        query_strings,
                ...
                Some(
                    ...
                    return Err(s3_error!(
                        NotImplemented,
                        "AWS4-ECDSA-P256-SHA256 signing method is not implemented yet"
                    ));
                }
                None => {
                    ...
                    if service == "sts" {
                        ...
                        let body_bytes = self
                            ...
                            .await
                            .map_err(|e| {
                            invalid_request!("failed to read STS request body: {}", e)
                        })?;
        ...
        if is_stream {
            ...
            let decoded_content_length = self.decoded_content_length.ok_or_else(|| {
                s3_error!(
                    MissingContentLength,
                    "missing header: x-amz-decoded-content-length"
                )
            })?;
            ...
            let length = if let Some(content_length) = self.content_length {
                usize::try_from(content_length)
                    .map_err(|_| invalid_request!("content-length exceeds platform limits"))?
            } else {
                self.req_body.remaining_length().exact().ok_or_else(|| {
                    s3_error!(MissingContentLength, "missing header: content-length")
                })?
            };
    ...
    pub async fn v2_check_header_auth(
        &mut self,
        auth_v2: AuthorizationV2<'_>,
    ) -> S3Result<CredentialsExt> {
        let method = &self.req_method;
        ...
        let date = self
            .hs
            .get_unique("date")
            .or_else(|| self.hs.get_unique("x-amz-date"));
        if date.is_none() {
    ...
    pub async fn v2_check_post_signature(
        &mut self,
        multipart: Multipart,
    ) -> S3Result<CredentialsExt> {
        let auth = require_auth(self.auth)?;

        let info = PostSignatureV2::extract(&multipart)
            .ok_or_else(|| invalid_request!("missing required multipart fields"))?;
    ...
    pub async fn v2_check_presigned_url(&mut self) -> S3Result<CredentialsExt> {
        ...
        let presigned_url = PresignedUrlV2::parse(qs)
            .map_err(|err| invalid_request!(err, "missing presigned url v2 fields"))?;
...
--{boundary}--\r\n"
        ...
        assert!(
            credentials.is_none(),
            "anonymous POST should not require credentials"
        );
    ...
    async fn v2_header_auth_returns_no_region() {
        ...
        let hs =
            OrderedHeaders::from_slice_unchecked(&[("date", date), ("host", "s3.amazonaws.com")]);
        ...
        assert_eq!(
            cred.service.as_deref(),
            Some("s3"),
            "SigV2 service is always 's3'"
        );
    }
}
