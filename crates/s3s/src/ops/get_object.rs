...
pub fn extract_overridden_response_headers(req: &S3Request<GetObjectInput>) -> S3Result<HeaderMap> {
    ...
    add(
        &mut map,
        header::CONTENT_TYPE,
        req.input.response_content_type.as_deref(),
    )?;
    add(
        &mut map,
        header::CONTENT_LANGUAGE,
        req.input.response_content_language.as_deref(),
    )?;
    add_ts(
        &mut map,
        header::EXPIRES,
        req.input.response_expires.as_ref(),
    )?;
    add(
        &mut map,
        header::CACHE_CONTROL,
        req.input.response_cache_control.as_deref(),
    )?;
    add(
        &mut map,
        header::CONTENT_DISPOSITION,
        req.input.response_content_disposition.as_deref(),
    )?;
    add(
        &mut map,
        header::CONTENT_ENCODING,
        req.input.response_content_encoding.as_deref(),
    )?;
...
fn add_ts(map: &mut HeaderMap, name: HeaderName, value: Option<&Timestamp>) -> S3Result<()> {
    let error = |e| invalid_request!(e, "invalid overridden header: {name}: {value:?}");
    if let Some(value) = value {
        let value = fmt_timestamp(value, TimestampFormat::HttpDate, HeaderValue::from_bytes)
            .map_err(error)?;
        map.insert(name, value);
    }
    Ok(())
