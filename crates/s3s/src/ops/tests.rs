...
async fn anonymous_request_clears_region_and_service() {
    ...
    assert_eq!(
        ...
        "anonymous request must clear region"
    );
    assert_eq!(
        req.s3ext.service, None,
        "anonymous request must clear service"
    );
}
...
async fn vh_region_fallback_for_anonymous_request() {
    ...
    impl S3Host for RegionHost {
        fn parse_host_header<'a>(&'a self, _host: &'a str) -> S3Result<VirtualHost<'a>> {
            Ok(VirtualHost::new("example.com")
                .with_bucket("bucket")
...
async fn post_multipart_bucket_routes_to_post_object() {
    ...
    impl crate::s3_trait::S3 for TestS3 {
        ...
        ) -> crate::error::S3Result<crate::protocol::S3Response<crate::dto::PutObjectOutput>>
        {
            ...
            Ok(crate::protocol::S3Response::new(
                crate::dto::PutObjectOutput::default(),
            ))
        }
        ...
        ) -> crate::error::S3Result<crate::protocol::S3Response<crate::dto::PostObjectOutput>>
        {
            ...
            Ok(crate::protocol::S3Response::new(
                crate::dto::PostObjectOutput::default(),
            ))
        }
    }
    ...
    let service = "s3";
    let signature =
        sig_v4::calculate_signature(policy_b64, &secret_key, &amz_date, region, service);
...
async fn post_object_policy_max_smaller_than_config_max() {
    ...
    let mut req = post_policy_test_helpers::build_post_object_request(
        policy_json,
        &file_content,
        &secret_key,
        true,
    ...
    assert!(
        result.is_ok(),
        "expected success for file within policy limit"
    );
}
...
async fn post_object_file_exceeds_policy_max_but_under_config_max() {
    ...
    assert!(
        result.is_err(),
        "expected error for file exceeding policy limit"
    );
...
async fn post_object_policy_max_larger_than_config_max() {
    ...
    assert!(
        result.is_ok(),
        "expected success for file within config limit"
    );
}
...
async fn post_object_content_length_range_rejects_oversized_file() {
    ...
    assert!(
        result.is_err(),
        "expected error for file exceeding content-length-range"
    );
...
mod access_control_test_helpers {
    ...
    impl crate::s3_trait::S3 for TestS3WithGetObject {
        ...
        ) -> crate::error::S3Result<crate::protocol::S3Response<crate::dto::GetObjectOutput>>
        {
            ...
            Ok(crate::protocol::S3Response::new(
                crate::dto::GetObjectOutput::default(),
            ))
        }
    }
}
...
async fn test_s3_route_custom_access_allows_anonymous() {
    ...
    match result {
        ...
        Err(err) => {
            panic!(
                "Anonymous request should succeed when custom access control allows it, got error: {err:?}"
            );
        }
    }
...
async fn test_custom_route_anonymous_access_denied() {
    ...
    impl S3Route for TestCustomRoute {
        ...
        async fn call(&self, _req: S3Request<Body>) -> crate::error::S3Result<S3Response<Body>> {
            ...
            Ok(S3Response::new(Body::from(
                "Custom route response".to_string(),
            )))
        }
    }
...
async fn test_custom_route_anonymous_access_allowed_when_overridden() {
    ...
    impl S3Route for AnonymousCustomRoute {
        ...
        async fn call(&self, _req: S3Request<Body>) -> crate::error::S3Result<S3Response<Body>> {
            self.call_count.fetch_add(1, Ordering::SeqCst);
            Ok(S3Response::new(Body::from(
                "Public route response".to_string(),
            )))
        }
    }
...
async fn test_s3_route_no_auth_provider_allows_unsigned_requests() {
    ...
    match result {
        ...
        Err(err) => {
            panic!(
                "Unsigned request should succeed when no auth provider is configured, got error: {err:?}"
            );
        }
    }
    ...
    assert_eq!(
        test_s3.get_call_count(),
        1,
        "S3 handler should have been invoked"
    );
}
...
async fn test_custom_route_override_check_access_allows_unsigned_requests() {
    ...
    let mut req = Request::from(
        hyper::Request::builder()
            ...
            .header(
                hyper::header::CONTENT_TYPE,
                HeaderValue::from_static("application/x-test"),
            )
            .body(Body::empty())
            .unwrap(),
    );
    ...
    match result {
        ...
        Err(err) => {
            panic!(
                "Unsigned request should succeed with overridden check_access, got error: {err:?}"
            );
        }
    }
    ...
    assert_eq!(
        test_route.get_call_count(),
        1,
        "Custom route should have been invoked"
    );
}
...
async fn test_custom_route_default_check_access_denies_unsigned_without_auth_provider() {
    ...
    let mut req = Request::from(
        hyper::Request::builder()
            ...
            .header(
                hyper::header::CONTENT_TYPE,
                HeaderValue::from_static("application/x-auth"),
            )
            .body(Body::empty())
            .unwrap(),
    );
    ...
    assert_eq!(
        test_route.get_call_count(),
        0,
        "Custom route handler should not have been invoked"
    );
}
...
async fn post_policy_file_size_is_total_bytes_not_chunk_count() {
    ...
    let mut req = post_policy_test_helpers::build_post_object_request_chunked(
        policy_json,
        &file_content,
        &secret_key,
        chunk_size,
    ...
    match result {
        Ok(_) => {}
        Err(err) => panic!(
            "POST object with 30 KB file should pass content-length-range [100, 50000] validation, got: {err:?}"
        ),
    }

    // Now test with a file that's too small (should fail)
    let small_file_content = "a".repeat(50); // 50 bytes, less than minimum of 100
    let mut req_small = post_policy_test_helpers::build_post_object_request_chunked(
        policy_json,
        &small_file_content,
        &secret_key,
        chunk_size,
...
fn create_session_deserialize_http() {
    ...
    assert_eq!(
        input
            .session_mode
            .as_ref()
            .map(crate::dto::SessionMode::as_str),
        Some("ReadWrite")
    );
...
fn create_session_serialize_http() {
    ...
    let creds = SessionCredentials {
        ...
        expiration: Timestamp::parse(TimestampFormat::DateTime, "2024-01-01T00:05:00.000Z")
            .unwrap(),
...
fn list_directory_buckets_deserialize_http() {
    ...
    req.s3ext.qs =
        Some(OrderedQs::parse("continuation-token=abc123&max-directory-buckets=10").unwrap());
