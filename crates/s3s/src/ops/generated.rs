...
impl AbortMultipartUpload {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<AbortMultipartUploadInput> {
        ...
        let expected_bucket_owner: Option<AccountId> =
            http::parse_opt_header(req, &X_AMZ_EXPECTED_BUCKET_OWNER)?;

        let if_match_initiated_time: Option<IfMatchInitiatedTime> =
            http::parse_opt_header_timestamp(
                req,
                &X_AMZ_IF_MATCH_INITIATED_TIME,
                TimestampFormat::HttpDate,
            )?;

        let request_payer: Option<RequestPayer> =
            http::parse_opt_header(req, &X_AMZ_REQUEST_PAYER)?;
...
impl CompleteMultipartUpload {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<CompleteMultipartUploadInput> {
        ...
        let checksum_crc32: Option<ChecksumCRC32> =
            http::parse_opt_header(req, &X_AMZ_CHECKSUM_CRC32)?;

        let checksum_crc32c: Option<ChecksumCRC32C> =
            http::parse_opt_header(req, &X_AMZ_CHECKSUM_CRC32C)?;

        let checksum_crc64nvme: Option<ChecksumCRC64NVME> =
            http::parse_opt_header(req, &X_AMZ_CHECKSUM_CRC64NVME)?;

        let checksum_sha1: Option<ChecksumSHA1> =
            http::parse_opt_header(req, &X_AMZ_CHECKSUM_SHA1)?;

        let checksum_sha256: Option<ChecksumSHA256> =
            http::parse_opt_header(req, &X_AMZ_CHECKSUM_SHA256)?;

        let checksum_type: Option<ChecksumType> =
            http::parse_opt_header(req, &X_AMZ_CHECKSUM_TYPE)?;
        ...
        let mpu_object_size: Option<MpuObjectSize> =
            http::parse_opt_header(req, &X_AMZ_MP_OBJECT_SIZE)?;
        ...
        let sse_customer_key: Option<SSECustomerKey> =
            http::parse_opt_header(req, &X_AMZ_SERVER_SIDE_ENCRYPTION_CUSTOMER_KEY)?;
...
impl CopyObject {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<CopyObjectInput> {
        ...
        let checksum_algorithm: Option<ChecksumAlgorithm> =
            http::parse_checksum_algorithm_header(req)?;

        let content_disposition: Option<ContentDisposition> =
            http::parse_opt_header(req, &CONTENT_DISPOSITION)?;

        let content_encoding: Option<ContentEncoding> =
            http::parse_opt_header(req, &CONTENT_ENCODING)?;

        let content_language: Option<ContentLanguage> =
            http::parse_opt_header(req, &CONTENT_LANGUAGE)?;
        ...
        let copy_source_if_match: Option<CopySourceIfMatch> =
            http::parse_opt_header(req, &X_AMZ_COPY_SOURCE_IF_MATCH)?;
        ...
        let expected_source_bucket_owner: Option<AccountId> =
            http::parse_opt_header(req, &X_AMZ_SOURCE_EXPECTED_BUCKET_OWNER)?;

        let expires: Option<Expires> =
            http::parse_opt_header_timestamp(req, &EXPIRES, TimestampFormat::HttpDate)?;

        let grant_full_control: Option<GrantFullControl> =
            http::parse_opt_header(req, &X_AMZ_GRANT_FULL_CONTROL)?;

        let grant_read: Option<GrantRead> = http::parse_opt_header(req, &X_AMZ_GRANT_READ)?;

        let grant_read_acp: Option<GrantReadACP> =
            http::parse_opt_header(req, &X_AMZ_GRANT_READ_ACP)?;

        let grant_write_acp: Option<GrantWriteACP> =
            http::parse_opt_header(req, &X_AMZ_GRANT_WRITE_ACP)?;

        let metadata: Option<Metadata> = http::parse_opt_metadata(req)?;

        let metadata_directive: Option<MetadataDirective> =
            http::parse_opt_header(req, &X_AMZ_METADATA_DIRECTIVE)?;

        let object_lock_legal_hold_status: Option<ObjectLockLegalHoldStatus> =
            http::parse_opt_header(req, &X_AMZ_OBJECT_LOCK_LEGAL_HOLD)?;

        let object_lock_mode: Option<ObjectLockMode> =
            http::parse_opt_header(req, &X_AMZ_OBJECT_LOCK_MODE)?;
        ...
        let ssekms_key_id: Option<SSEKMSKeyId> =
            http::parse_opt_header(req, &X_AMZ_SERVER_SIDE_ENCRYPTION_AWS_KMS_KEY_ID)?;

        let server_side_encryption: Option<ServerSideEncryption> =
            http::parse_opt_header(req, &X_AMZ_SERVER_SIDE_ENCRYPTION)?;

        let storage_class: Option<StorageClass> =
            http::parse_opt_header(req, &X_AMZ_STORAGE_CLASS)?;

        let tagging: Option<TaggingHeader> = http::parse_opt_header(req, &X_AMZ_TAGGING)?;

        let tagging_directive: Option<TaggingDirective> =
            http::parse_opt_header(req, &X_AMZ_TAGGING_DIRECTIVE)?;
...
impl CreateBucket {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<CreateBucketInput> {
        ...
        let create_bucket_configuration: Option<CreateBucketConfiguration> =
            http::take_opt_xml_body(req)?;
        ...
        let object_ownership: Option<ObjectOwnership> =
            http::parse_opt_header(req, &X_AMZ_OBJECT_OWNERSHIP)?;
...
impl CreateBucketMetadataTableConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<CreateBucketMetadataTableConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
    ...
    pub fn serialize_http(
        _: CreateBucketMetadataTableConfigurationOutput,
    ) -> S3Result<http::Response> {
...
impl super::Operation for CreateBucketMetadataTableConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .create_bucket_metadata_table_configuration(&mut s3_req)
                .await?;
        }
...
impl CreateSession {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<CreateSessionInput> {
        ...
        let session_mode: Option<SessionMode> =
            http::parse_opt_header(req, &X_AMZ_CREATE_SESSION_MODE)?;
...
impl DeleteBucketAnalyticsConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<DeleteBucketAnalyticsConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for DeleteBucketAnalyticsConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .delete_bucket_analytics_configuration(&mut s3_req)
                .await?;
        }
...
impl DeleteBucketIntelligentTieringConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<DeleteBucketIntelligentTieringConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
    ...
    pub fn serialize_http(
        _: DeleteBucketIntelligentTieringConfigurationOutput,
    ) -> S3Result<http::Response> {
        Ok(http::Response::with_status(http::StatusCode::NO_CONTENT))
    }
}
...
impl super::Operation for DeleteBucketIntelligentTieringConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .delete_bucket_intelligent_tiering_configuration(&mut s3_req)
                .await?;
        }
        let result = s3
            .delete_bucket_intelligent_tiering_configuration(s3_req)
            .await;
        let s3_resp = match result {
            Ok(val) => val,
            Err(err) => return super::serialize_error(err, false),
...
impl DeleteBucketInventoryConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<DeleteBucketInventoryConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for DeleteBucketInventoryConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .delete_bucket_inventory_configuration(&mut s3_req)
                .await?;
        }
...
impl DeleteBucketMetadataTableConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<DeleteBucketMetadataTableConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
    ...
    pub fn serialize_http(
        _: DeleteBucketMetadataTableConfigurationOutput,
    ) -> S3Result<http::Response> {
        Ok(http::Response::with_status(http::StatusCode::NO_CONTENT))
    }
}
...
impl super::Operation for DeleteBucketMetadataTableConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .delete_bucket_metadata_table_configuration(&mut s3_req)
                .await?;
        }
...
impl DeleteBucketMetricsConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<DeleteBucketMetricsConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for DeleteBucketMetricsConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .delete_bucket_metrics_configuration(&mut s3_req)
                .await?;
        }
...
impl DeleteBucketOwnershipControls {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<DeleteBucketOwnershipControlsInput> {
        let bucket = http::unwrap_bucket(req);
...
impl DeleteObject {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<DeleteObjectInput> {
        ...
        let if_match_last_modified_time: Option<IfMatchLastModifiedTime> =
            http::parse_opt_header_timestamp(
                req,
                &X_AMZ_IF_MATCH_LAST_MODIFIED_TIME,
                TimestampFormat::HttpDate,
            )?;
...
impl GetBucketAccelerateConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketAccelerateConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for GetBucketAccelerateConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .get_bucket_accelerate_configuration(&mut s3_req)
                .await?;
        }
...
impl GetBucketAnalyticsConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketAnalyticsConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for GetBucketAnalyticsConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .get_bucket_analytics_configuration(&mut s3_req)
                .await?;
        }
...
impl GetBucketIntelligentTieringConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketIntelligentTieringConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
    ...
    pub fn serialize_http(
        x: GetBucketIntelligentTieringConfigurationOutput,
    ) -> S3Result<http::Response> {
        let mut res = http::Response::with_status(http::StatusCode::OK);
...
impl super::Operation for GetBucketIntelligentTieringConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .get_bucket_intelligent_tiering_configuration(&mut s3_req)
                .await?;
        }
        let result = s3
            .get_bucket_intelligent_tiering_configuration(s3_req)
            .await;
        let s3_resp = match result {
            Ok(val) => val,
            Err(err) => return super::serialize_error(err, false),
...
impl GetBucketInventoryConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketInventoryConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for GetBucketInventoryConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .get_bucket_inventory_configuration(&mut s3_req)
                .await?;
        }
...
impl GetBucketLifecycleConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketLifecycleConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for GetBucketLifecycleConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .get_bucket_lifecycle_configuration(&mut s3_req)
                .await?;
        }
...
impl GetBucketMetadataTableConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketMetadataTableConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
    ...
    pub fn serialize_http(
        x: GetBucketMetadataTableConfigurationOutput,
    ) -> S3Result<http::Response> {
        let mut res = http::Response::with_status(http::StatusCode::OK);
...
impl super::Operation for GetBucketMetadataTableConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .get_bucket_metadata_table_configuration(&mut s3_req)
                .await?;
        }
...
impl GetBucketMetricsConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketMetricsConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl GetBucketNotificationConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<GetBucketNotificationConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for GetBucketNotificationConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .get_bucket_notification_configuration(&mut s3_req)
                .await?;
        }
...
impl GetObject {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<GetObjectInput> {
        ...
        let checksum_mode: Option<ChecksumMode> =
            http::parse_opt_header(req, &X_AMZ_CHECKSUM_MODE)?;
        ...
        let response_cache_control: Option<ResponseCacheControl> =
            http::parse_opt_query(req, "response-cache-control")?;

        let response_content_disposition: Option<ResponseContentDisposition> =
            http::parse_opt_query(req, "response-content-disposition")?;

        let response_content_encoding: Option<ResponseContentEncoding> =
            http::parse_opt_query(req, "response-content-encoding")?;

        let response_content_language: Option<ResponseContentLanguage> =
            http::parse_opt_query(req, "response-content-language")?;

        let response_content_type: Option<ResponseContentType> =
            http::parse_opt_query(req, "response-content-type")?;
    ...
    pub fn serialize_http(x: GetObjectOutput) -> S3Result<http::Response> {
        ...
        http::add_opt_header(
            ...
            x.object_lock_legal_hold_status,
        )?;
...
impl GetObjectAttributes {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<GetObjectAttributesInput> {
        ...
        let object_attributes: ObjectAttributesList =
            http::parse_list_header(req, &X_AMZ_OBJECT_ATTRIBUTES)?;

        let part_number_marker: Option<PartNumberMarker> =
            http::parse_opt_header(req, &X_AMZ_PART_NUMBER_MARKER)?;
...
impl ListBucketAnalyticsConfigurations {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<ListBucketAnalyticsConfigurationsInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for ListBucketAnalyticsConfigurations {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .list_bucket_analytics_configurations(&mut s3_req)
                .await?;
        }
...
impl ListBucketIntelligentTieringConfigurations {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<ListBucketIntelligentTieringConfigurationsInput> {
        let bucket = http::unwrap_bucket(req);
    ...
    pub fn serialize_http(
        x: ListBucketIntelligentTieringConfigurationsOutput,
    ) -> S3Result<http::Response> {
        let mut res = http::Response::with_status(http::StatusCode::OK);
        http::set_xml_body(&mut res, &x)?;
        Ok(res)
...
impl super::Operation for ListBucketIntelligentTieringConfigurations {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .list_bucket_intelligent_tiering_configurations(&mut s3_req)
                .await?;
        }
        let result = s3
            .list_bucket_intelligent_tiering_configurations(s3_req)
            .await;
        let s3_resp = match result {
            Ok(val) => val,
            Err(err) => return super::serialize_error(err, false),
...
impl ListBucketInventoryConfigurations {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<ListBucketInventoryConfigurationsInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for ListBucketInventoryConfigurations {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .list_bucket_inventory_configurations(&mut s3_req)
                .await?;
        }
...
impl ListBucketMetricsConfigurations {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<ListBucketMetricsConfigurationsInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for ListBucketMetricsConfigurations {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .list_bucket_metrics_configurations(&mut s3_req)
                .await?;
        }
...
impl ListDirectoryBuckets {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<ListDirectoryBucketsInput> {
        let continuation_token: Option<DirectoryBucketToken> =
            http::parse_opt_query(req, "continuation-token")?;

        let max_directory_buckets: Option<MaxDirectoryBuckets> =
            http::parse_opt_query(req, "max-directory-buckets")?;
...
impl ListMultipartUploads {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<ListMultipartUploadsInput> {
        ...
        let upload_id_marker: Option<UploadIdMarker> =
            http::parse_opt_query(req, "upload-id-marker")?;
...
impl ListObjectVersions {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<ListObjectVersionsInput> {
        ...
        let version_id_marker: Option<VersionIdMarker> =
            http::parse_opt_query(req, "version-id-marker")?;
...
impl ListParts {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<ListPartsInput> {
        ...
        let part_number_marker: Option<PartNumberMarker> =
            http::parse_opt_query(req, "part-number-marker")?;
...
impl PutBucketAccelerateConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<PutBucketAccelerateConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for PutBucketAccelerateConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .put_bucket_accelerate_configuration(&mut s3_req)
                .await?;
        }
...
impl PutBucketAnalyticsConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<PutBucketAnalyticsConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for PutBucketAnalyticsConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .put_bucket_analytics_configuration(&mut s3_req)
                .await?;
        }
...
impl PutBucketIntelligentTieringConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<PutBucketIntelligentTieringConfigurationInput> {
        let bucket = http::unwrap_bucket(req);

        let id: IntelligentTieringId = http::parse_query(req, "id")?;

        let intelligent_tiering_configuration: IntelligentTieringConfiguration =
            http::take_xml_body(req)?;
    ...
    pub fn serialize_http(
        _: PutBucketIntelligentTieringConfigurationOutput,
    ) -> S3Result<http::Response> {
        Ok(http::Response::with_status(http::StatusCode::OK))
    }
}
...
impl super::Operation for PutBucketIntelligentTieringConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .put_bucket_intelligent_tiering_configuration(&mut s3_req)
                .await?;
        }
        let result = s3
            .put_bucket_intelligent_tiering_configuration(s3_req)
            .await;
        let s3_resp = match result {
            Ok(val) => val,
            Err(err) => return super::serialize_error(err, false),
...
impl PutBucketInventoryConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<PutBucketInventoryConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for PutBucketInventoryConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .put_bucket_inventory_configuration(&mut s3_req)
                .await?;
        }
...
impl PutBucketLifecycleConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<PutBucketLifecycleConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
        ...
        let lifecycle_configuration: Option<BucketLifecycleConfiguration> =
            http::take_opt_xml_body(req)?;
...
impl super::Operation for PutBucketLifecycleConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .put_bucket_lifecycle_configuration(&mut s3_req)
                .await?;
        }
...
impl PutBucketMetricsConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<PutBucketMetricsConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl PutBucketNotificationConfiguration {
    pub fn deserialize_http(
        req: &mut http::Request,
    ) -> S3Result<PutBucketNotificationConfigurationInput> {
        let bucket = http::unwrap_bucket(req);
...
impl super::Operation for PutBucketNotificationConfiguration {
    ...
    async fn call(
        ...
        if let Some(access) = ccx.access {
            access
                .put_bucket_notification_configuration(&mut s3_req)
                .await?;
        }
...
impl PutBucketReplication {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<PutBucketReplicationInput> {
        ...
        let token: Option<ObjectLockToken> =
            http::parse_opt_header(req, &X_AMZ_BUCKET_OBJECT_LOCK_TOKEN)?;
...
impl PutObject {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<PutObjectInput> {
        ...
        let write_offset_bytes: Option<WriteOffsetBytes> =
            http::parse_opt_header(req, &X_AMZ_WRITE_OFFSET_BYTES)?;
    ...
    pub fn deserialize_http_multipart(
        req: &mut http::Request,
        m: http::Multipart,
    ) -> S3Result<PutObjectInput> {
        let bucket = http::unwrap_bucket(req);
        let key =
            http::parse_field_value(&m, "key")?.ok_or_else(|| invalid_request!("missing key"))?;
        ...
        let checksum_algorithm: Option<ChecksumAlgorithm> =
            http::parse_field_value(&m, "x-amz-sdk-checksum-algorithm")?;

        let checksum_crc32: Option<ChecksumCRC32> =
            http::parse_field_value(&m, "x-amz-checksum-crc32")?;

        let checksum_crc32c: Option<ChecksumCRC32C> =
            http::parse_field_value(&m, "x-amz-checksum-crc32c")?;

        let checksum_crc64nvme: Option<ChecksumCRC64NVME> =
            http::parse_field_value(&m, "x-amz-checksum-crc64nvme")?;

        let checksum_sha1: Option<ChecksumSHA1> =
            http::parse_field_value(&m, "x-amz-checksum-sha1")?;

        let checksum_sha256: Option<ChecksumSHA256> =
            http::parse_field_value(&m, "x-amz-checksum-sha256")?;

        let content_disposition: Option<ContentDisposition> =
            http::parse_field_value(&m, "content-disposition")?;

        let content_encoding: Option<ContentEncoding> =
            http::parse_field_value(&m, "content-encoding")?;

        let content_language: Option<ContentLanguage> =
            http::parse_field_value(&m, "content-language")?;
        ...
        let expected_bucket_owner: Option<AccountId> =
            http::parse_field_value(&m, "x-amz-expected-bucket-owner")?;

        let expires: Option<Expires> =
            http::parse_field_value_timestamp(&m, "expires", TimestampFormat::HttpDate)?;

        let grant_full_control: Option<GrantFullControl> =
            http::parse_field_value(&m, "x-amz-grant-full-control")?;

        let grant_read: Option<GrantRead> = http::parse_field_value(&m, "x-amz-grant-read")?;

        let grant_read_acp: Option<GrantReadACP> =
            http::parse_field_value(&m, "x-amz-grant-read-acp")?;

        let grant_write_acp: Option<GrantWriteACP> =
            http::parse_field_value(&m, "x-amz-grant-write-acp")?;
        ...
        let object_lock_mode: Option<ObjectLockMode> =
            http::parse_field_value(&m, "x-amz-object-lock-mode")?;

        let object_lock_retain_until_date: Option<ObjectLockRetainUntilDate> =
            http::parse_field_value_timestamp(
                &m,
        ...
        let request_payer: Option<RequestPayer> =
            http::parse_field_value(&m, "x-amz-request-payer")?;

        let sse_customer_algorithm: Option<SSECustomerAlgorithm> =
            http::parse_field_value(&m, "x-amz-server-side-encryption-customer-algorithm")?;

        let sse_customer_key: Option<SSECustomerKey> =
            http::parse_field_value(&m, "x-amz-server-side-encryption-customer-key")?;
        ...
        let ssekms_key_id: Option<SSEKMSKeyId> =
            http::parse_field_value(&m, "x-amz-server-side-encryption-aws-kms-key-id")?;

        let server_side_encryption: Option<ServerSideEncryption> =
            http::parse_field_value(&m, "x-amz-server-side-encryption")?;

        let storage_class: Option<StorageClass> =
            http::parse_field_value(&m, "x-amz-storage-class")?;
        ...
        let write_offset_bytes: Option<WriteOffsetBytes> =
            http::parse_field_value(&m, "x-amz-write-offset-bytes")?;
...
impl UploadPartCopy {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<UploadPartCopyInput> {
        ...
        let copy_source_range: Option<CopySourceRange> =
            http::parse_opt_header(req, &X_AMZ_COPY_SOURCE_RANGE)?;
...
impl WriteGetObjectResponse {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<WriteGetObjectResponseInput> {
        let accept_ranges: Option<AcceptRanges> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_ACCEPT_RANGES)?;

        let body: Option<StreamingBlob> = Some(http::take_stream_body(req));

        let bucket_key_enabled: Option<BucketKeyEnabled> = http::parse_opt_header(
            req,
            &X_AMZ_FWD_HEADER_X_AMZ_SERVER_SIDE_ENCRYPTION_BUCKET_KEY_ENABLED,
        )?;

        let cache_control: Option<CacheControl> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_CACHE_CONTROL)?;

        let checksum_crc32: Option<ChecksumCRC32> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_CHECKSUM_CRC32)?;

        let checksum_crc32c: Option<ChecksumCRC32C> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_CHECKSUM_CRC32C)?;

        let checksum_crc64nvme: Option<ChecksumCRC64NVME> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_CHECKSUM_CRC64NVME)?;

        let checksum_sha1: Option<ChecksumSHA1> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_CHECKSUM_SHA1)?;

        let checksum_sha256: Option<ChecksumSHA256> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_CHECKSUM_SHA256)?;

        let content_disposition: Option<ContentDisposition> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_CONTENT_DISPOSITION)?;

        let content_encoding: Option<ContentEncoding> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_CONTENT_ENCODING)?;

        let content_language: Option<ContentLanguage> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_CONTENT_LANGUAGE)?;

        let content_length: Option<ContentLength> = http::parse_opt_header(req, &CONTENT_LENGTH)?;

        let content_range: Option<ContentRange> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_CONTENT_RANGE)?;

        let content_type: Option<ContentType> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_CONTENT_TYPE)?;

        let delete_marker: Option<DeleteMarker> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_DELETE_MARKER)?;
        ...
        let error_message: Option<ErrorMessage> =
            http::parse_opt_header(req, &X_AMZ_FWD_ERROR_MESSAGE)?;

        let expiration: Option<Expiration> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_EXPIRATION)?;

        let expires: Option<Expires> = http::parse_opt_header_timestamp(
            req,
            &X_AMZ_FWD_HEADER_EXPIRES,
            TimestampFormat::HttpDate,
        )?;

        let last_modified: Option<LastModified> = http::parse_opt_header_timestamp(
            req,
            &X_AMZ_FWD_HEADER_LAST_MODIFIED,
            TimestampFormat::HttpDate,
        )?;

        let metadata: Option<Metadata> = http::parse_opt_metadata(req)?;

        let missing_meta: Option<MissingMeta> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_MISSING_META)?;

        let object_lock_legal_hold_status: Option<ObjectLockLegalHoldStatus> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_OBJECT_LOCK_LEGAL_HOLD)?;

        let object_lock_mode: Option<ObjectLockMode> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_OBJECT_LOCK_MODE)?;
        ...
        let parts_count: Option<PartsCount> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_MP_PARTS_COUNT)?;

        let replication_status: Option<ReplicationStatus> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_REPLICATION_STATUS)?;

        let request_charged: Option<RequestCharged> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_REQUEST_CHARGED)?;
        ...
        let restore: Option<Restore> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_RESTORE)?;

        let sse_customer_algorithm: Option<SSECustomerAlgorithm> = http::parse_opt_header(
            req,
            &X_AMZ_FWD_HEADER_X_AMZ_SERVER_SIDE_ENCRYPTION_CUSTOMER_ALGORITHM,
        )?;

        let sse_customer_key_md5: Option<SSECustomerKeyMD5> = http::parse_opt_header(
            req,
            &X_AMZ_FWD_HEADER_X_AMZ_SERVER_SIDE_ENCRYPTION_CUSTOMER_KEY_MD5,
        )?;

        let ssekms_key_id: Option<SSEKMSKeyId> = http::parse_opt_header(
            req,
            &X_AMZ_FWD_HEADER_X_AMZ_SERVER_SIDE_ENCRYPTION_AWS_KMS_KEY_ID,
        )?;

        let server_side_encryption: Option<ServerSideEncryption> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_SERVER_SIDE_ENCRYPTION)?;

        let status_code: Option<GetObjectResponseStatusCode> =
            http::parse_opt_header(req, &X_AMZ_FWD_STATUS)?;

        let storage_class: Option<StorageClass> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_STORAGE_CLASS)?;

        let tag_count: Option<TagCount> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_TAGGING_COUNT)?;

        let version_id: Option<ObjectVersionId> =
            http::parse_opt_header(req, &X_AMZ_FWD_HEADER_X_AMZ_VERSION_ID)?;
...
impl PostObject {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<PostObjectInput> {
        ...
        let success_action_redirect: Option<String> =
            match http::parse_field_value(&m, "success_action_redirect")? {
                Some(v) => Some(v),
                None => http::parse_field_value(&m, "redirect")?,
            };
        let success_action_status: Option<i32> =
            http::parse_field_value(&m, "success_action_status")?;
    ...
    pub fn serialize_http(
        ...
        if let Some(redirect_url) = success_action_redirect {
            ...
            if redirect_url.chars().any(char::is_control) {
                return Err(s3_error!(
                    InvalidArgument,
                    "success_action_redirect contains invalid control characters"
                ));
            }

            // Parse the URL to validate and manipulate it properly
            let mut url = url::Url::parse(redirect_url)
                .map_err(|e| s3_error!(e, InvalidArgument, "Invalid redirect URL"))?;
...
impl super::Operation for PostObject {
    ...
    async fn call(
        ...
        post_req
            ...
            .clone_from(&success_action_redirect);
            ...
            S3Path::Root => {
                if let Some(qs) = qs {
                    if super::check_query_pattern(qs, "x-id", "ListDirectoryBuckets") {
                        return Ok((
                            &ListDirectoryBuckets as &'static dyn super::Operation,
                            false,
                        ));
                    }
                }
            ...
            S3Path::Bucket { .. } => {
                if let Some(qs) = qs {
                    if qs.has("analytics") && qs.has("id") {
                        return Ok((
                            &GetBucketAnalyticsConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("intelligent-tiering") && qs.has("id") {
                        return Ok((
                            &GetBucketIntelligentTieringConfiguration
                                as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("inventory") && qs.has("id") {
                        return Ok((
                            &GetBucketInventoryConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("metrics") && qs.has("id") {
                        return Ok((
                            &GetBucketMetricsConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("session") {
                    ...
                    if qs.has("accelerate") {
                        return Ok((
                            &GetBucketAccelerateConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("acl") {
                    ...
                    if qs.has("lifecycle") {
                        return Ok((
                            &GetBucketLifecycleConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("location") {
                    ...
                    if qs.has("metadataTable") {
                        return Ok((
                            &GetBucketMetadataTableConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("notification") {
                        return Ok((
                            &GetBucketNotificationConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("ownershipControls") {
                        return Ok((
                            &GetBucketOwnershipControls as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("policy") {
                    ...
                    if qs.has("policyStatus") {
                        return Ok((
                            &GetBucketPolicyStatus as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("replication") {
                        return Ok((
                            &GetBucketReplication as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("requestPayment") {
                        return Ok((
                            &GetBucketRequestPayment as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("tagging") {
                    ...
                    if qs.has("object-lock") {
                        return Ok((
                            &GetObjectLockConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("publicAccessBlock") {
                        return Ok((
                            &GetPublicAccessBlock as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("analytics") && !qs.has("id") {
                        return Ok((
                            &ListBucketAnalyticsConfigurations as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("intelligent-tiering") && !qs.has("id") {
                    ...
                    if qs.has("inventory") && !qs.has("id") {
                        return Ok((
                            &ListBucketInventoryConfigurations as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("metrics") && !qs.has("id") {
                        return Ok((
                            &ListBucketMetricsConfigurations as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("uploads") {
                        return Ok((
                            &ListMultipartUploads as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("versions") {
        ...
        hyper::Method::POST => match s3_path {
            ...
            S3Path::Bucket { .. } => {
                ...
                if req.headers.contains_key("x-amz-request-route")
                    && req.headers.contains_key("x-amz-request-token")
                {
                    return Ok((
                        &WriteGetObjectResponse as &'static dyn super::Operation,
                        false,
                    ));
                }
            ...
            S3Path::Object { .. } => {
                if let Some(qs) = qs {
                    ...
                    if qs.has("uploads") {
                        return Ok((
                            &CreateMultipartUpload as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("restore") {
                ...
                {
                    return Ok((
                        &CompleteMultipartUpload as &'static dyn super::Operation,
                        true,
                    ));
                }
                Err(super::unknown_operation())
        ...
        hyper::Method::PUT => match s3_path {
            ...
            S3Path::Bucket { .. } => {
                if let Some(qs) = qs {
                    if qs.has("analytics") {
                        return Ok((
                            &PutBucketAnalyticsConfiguration as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    ...
                    if qs.has("inventory") {
                        return Ok((
                            &PutBucketInventoryConfiguration as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("metrics") {
                        return Ok((
                            &PutBucketMetricsConfiguration as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("accelerate") {
                        return Ok((
                            &PutBucketAccelerateConfiguration as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("acl") {
                    ...
                    if qs.has("lifecycle") {
                        return Ok((
                            &PutBucketLifecycleConfiguration as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("logging") {
                    ...
                    if qs.has("notification") {
                        return Ok((
                            &PutBucketNotificationConfiguration as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("ownershipControls") {
                        return Ok((
                            &PutBucketOwnershipControls as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("policy") {
                    ...
                    if qs.has("requestPayment") {
                        return Ok((
                            &PutBucketRequestPayment as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("tagging") {
                    ...
                    if qs.has("object-lock") {
                        return Ok((
                            &PutObjectLockConfiguration as &'static dyn super::Operation,
                            true,
                        ));
                    }
                    if qs.has("publicAccessBlock") {
        ...
        hyper::Method::DELETE => match s3_path {
            ...
            S3Path::Bucket { .. } => {
                if let Some(qs) = qs {
                    if qs.has("analytics") {
                        return Ok((
                            &DeleteBucketAnalyticsConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("intelligent-tiering") {
                    ...
                    if qs.has("inventory") {
                        return Ok((
                            &DeleteBucketInventoryConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("metrics") {
                        return Ok((
                            &DeleteBucketMetricsConfiguration as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("cors") {
                    ...
                    if qs.has("encryption") {
                        return Ok((
                            &DeleteBucketEncryption as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("lifecycle") {
                        return Ok((
                            &DeleteBucketLifecycle as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("metadataTable") {
                    ...
                    if qs.has("ownershipControls") {
                        return Ok((
                            &DeleteBucketOwnershipControls as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("policy") {
                    ...
                    if qs.has("replication") {
                        return Ok((
                            &DeleteBucketReplication as &'static dyn super::Operation,
                            false,
                        ));
                    }
                    if qs.has("tagging") {
                    ...
                    if qs.has("publicAccessBlock") {
                        return Ok((
                            &DeletePublicAccessBlock as &'static dyn super::Operation,
                            false,
                        ));
                    }
                }
            ...
            S3Path::Object { .. } => {
                ...
                {
                    return Ok((
                        &AbortMultipartUpload as &'static dyn super::Operation,
                        false,
                    ));
                }
