...
fn get_optional_header<'r>(
    req: &'r Request,
    name: &HeaderName,
) -> S3Result<Option<&'r HeaderValue>> {
    let mut iter = req.headers.get_all(name).into_iter();
    let Some(val) = iter.next() else {
        return Ok(None);
    };
...
where
    ...
    T::Error: std::error::Error + Send + Sync + 'static,
{
    let Some(val) = get_optional_header(req, name)? else {
        return Ok(None);
    };
...
pub fn parse_checksum_algorithm_header(
    req: &Request,
) -> S3Result<Option<crate::dto::ChecksumAlgorithm>> {
    let ans: Option<crate::dto::ChecksumAlgorithm> =
        parse_opt_header(req, &crate::header::X_AMZ_CHECKSUM_ALGORITHM)?;
    ...
    let mapping = &const {
        [
            (
                ...
                crate::dto::ChecksumAlgorithm::CRC32,
            ),
            (
                crate::header::X_AMZ_CHECKSUM_CRC32C,
                crate::dto::ChecksumAlgorithm::CRC32C,
            ),
            (
                crate::header::X_AMZ_CHECKSUM_SHA1,
                crate::dto::ChecksumAlgorithm::SHA1,
            ),
            (
                crate::header::X_AMZ_CHECKSUM_SHA256,
                crate::dto::ChecksumAlgorithm::SHA256,
            ),
            (
                crate::header::X_AMZ_CHECKSUM_CRC64NVME,
                crate::dto::ChecksumAlgorithm::CRC64NVME,
            ),
        ]
    };
...
pub fn parse_opt_header_timestamp(
    req: &Request,
    name: &HeaderName,
    fmt: TimestampFormat,
) -> S3Result<Option<Timestamp>> {
    let Some(val) = get_optional_header(req, name)? else {
        return Ok(None);
    };
    ...
    T::Err: std::error::Error + Send + Sync + 'static,
{
    ...
    val.parse::<T>()
        .map_err(|err| invalid_query(err, name, val))
}
...
pub fn parse_opt_query_timestamp(
    req: &Request,
    name: &str,
    fmt: TimestampFormat,
) -> S3Result<Option<Timestamp>> {
    ...
    Ok(Some(
        Timestamp::parse(fmt, val).map_err(|err| invalid_query(err, name, val))?,
    ))
}
...
pub fn parse_opt_metadata(req: &Request) -> S3Result<Option<Metadata>> {
    ...
    for name in map.keys() {
        ...
        let raw =
            std::str::from_utf8(val.as_bytes()).map_err(|err| invalid_header(err, name, val))?;
...
pub fn parse_field_value_timestamp(
    m: &Multipart,
    name: &str,
    fmt: TimestampFormat,
) -> S3Result<Option<Timestamp>> {
