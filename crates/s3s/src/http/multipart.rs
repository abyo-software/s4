...
pub enum MultipartError {
    ...
    #[error(
        "MultipartError: TotalSizeTooLarge: total form fields size {0} bytes exceeds limit of {1} bytes"
    )]
...
pub async fn aggregate_file_stream_limited(
    mut stream: FileStream,
    max_size: u64,
) -> Result<Vec<Bytes>, MultipartError> {
    ...
    loop {
        ...
        *parts_count += 1;
        if *parts_count > limits.max_parts {
            return Ok(Err(MultipartError::TooManyParts(
                *parts_count,
                limits.max_parts,
            )));
        }
        ...
        if content_disposition.name.eq_ignore_ascii_case("file") {
            ...
            let file_name = content_disposition
                .filename
                .unwrap_or(content_disposition.name);
            let file = File {
        ...
        let value = match lines.split_to(pat_without_crlf) {
            ...
            Some(b) => {
                ...
                if b.len() > limits.max_field_size {
                    return Ok(Err(MultipartError::FieldTooLarge(
                        b.len(),
                ...
                *total_fields_size = total_fields_size.saturating_add(b.len());
                if *total_fields_size > limits.max_fields_size {
                    return Ok(Err(MultipartError::TotalSizeTooLarge(
                        *total_fields_size,
                        limits.max_fields_size,
                    )));
                }
...
mod tests {
    ...
    async fn multipart() {
        ...
        let file_bytes = aggregate_file_stream(ans.file.stream.unwrap())
            .await
            .unwrap();
    ...
    async fn post_object() {
        ...
        let body_bytes: Vec<Result<Bytes, StdError>> = {
            bytes
                .iter()
                .copied()
    ...
    async fn multipart_field_with_filename() {
        ...
        let body_stream = futures::stream::iter(
            body_bytes
                .into_iter()
                .map(|s| Ok::<_, StdError>(Bytes::from(s))),
        );
    ...
    async fn test_limits_within_bounds() {
        ...
        let result =
            transform_multipart(body_stream, boundary.as_bytes(), MultipartLimits::default()).await;
    ...
    async fn test_boundary_buffer_too_large() {
        ...
        let mut file_stream = multipart
            .take_file_stream()
            .expect("File stream should exist");
