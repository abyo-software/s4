...
pub struct AwsChunkedStream {
    ...
    inner: AsyncTryStream<
        ...
        SyncBoxFuture<'static, Result<(), AwsChunkedStreamError>>,
    >,
...
fn check_signature(
    ...
    let string_to_sign = sig_v4::create_chunk_string_to_sign(
        &ctx.amz_date,
        &ctx.region,
        &ctx.service,
        &ctx.prev_signature,
    ...
    let chunk_signature = sig_v4::calculate_signature(
        &string_to_sign,
        &ctx.secret_key,
        &ctx.amz_date,
        &ctx.region,
...
impl AwsChunkedStream {
    ...
    ///
    /// v0.13.0-s4.1: `skip_verify` controls whether per-chunk + trailer
    /// signatures are checked when the stream is NOT marked `unsigned`. When
    /// `true`, signatures parsed off the chunk meta lines are accepted
    /// without an HMAC compare (NoVerify mode mirroring
    /// [`S3ServiceBuilder::set_skip_signature_verification`]). `unsigned`
    /// continues to govern whether signatures are *required* on each chunk.
    ///
        ...
        S: Stream<Item = Result<Bytes, StdError>> + Send + Sync + 'static,
    {
        ...
        let inner =
            AsyncTryStream::<_, _, SyncBoxFuture<'static, Result<(), AwsChunkedStreamError>>>::new(
                |mut y| {
                        ...
                        loop {
                            let meta = {
                                    ...
                                    .await
                                {
                                    None => break,
                                    Some(Err(e)) => {
                                        return Err(AwsChunkedStreamError::Underlying(e));
                                    }
                            ...
                            if let Some(expected_sig) = meta.signature {
                                match check_signature(&ctx, expected_sig, &data) {
                                    None => {
                                        if !skip_verify {
                                            return Err(AwsChunkedStreamError::SignatureMismatch);
                                        }
                                        // v0.13.0-s4.1 NoVerify: leave `prev_signature` untouched
                                        // and let the next chunk's verification fall through the
                                        // same skip_verify branch. Trailer verification (later in
                                        // the loop) honours skip_verify too, so the unchanged
                                        // rolling state never becomes a comparison input.
                                        tracing::debug!(
                                            expected = ?std::str::from_utf8(expected_sig).unwrap_or("<non-utf8>"),
                                            "chunk signature mismatch (NoVerify mode — accepted)"
                                        );
                                    }
                            ...
                            if meta.size == 0 {
                                ...
                                match Self::read_and_verify_trailers(
                                    body.as_mut(),
                                    remaining_after_data,
                                    &ctx,
                                    unsigned,
                                    skip_verify,
                                )
        ...
        S: Stream<Item = Result<Bytes, StdError>> + Send + 'static,
    {
        ...
        if prev_bytes.is_empty() {
            ...
            if total_size > MAX_TRAILERS_SIZE {
                return Some(Err(AwsChunkedStreamError::TrailersTooLarge(
                    total_size,
                    MAX_TRAILERS_SIZE,
                )));
            }
        ...
        let (canonical_trailers, provided_signature, entries) =
            match Self::parse_trailing_headers(&buf) {
                Ok(x) => x,
        ...
        if let Some(provided) = provided_signature.as_ref() {
            ...
            let trailer_signature = sig_v4::calculate_signature(
                &string_to_sign,
                &ctx.secret_key,
                &ctx.amz_date,
                &ctx.region,
            ...
            if trailer_signature.as_bytes() != provided.as_slice() && !skip_verify {
    ...
    fn parse_trailing_headers(
        buf: &[u8],
    ) -> Result<(Vec<u8>, Option<Vec<u8>>, Vec<(String, String)>), AwsChunkedStreamError> {
        ...
        for i in 0..=buf.len() {
            ...
            if is_lf || is_end {
                ...
                if header_count > MAX_TRAILER_HEADERS {
                    return Err(AwsChunkedStreamError::TooManyTrailerHeaders(
                        header_count,
                        MAX_TRAILER_HEADERS,
                    ));
                }
                ...
                // Lowercase name
                let name = String::from_utf8(name_raw.to_ascii_lowercase())
                    .map_err(|_| AwsChunkedStreamError::FormatError)?;
    ...
    fn poll(
        ...
        cx: &mut Context<'_>,
    ) -> Poll<Option<Result<Bytes, AwsChunkedStreamError>>> {
...
mod tests {
    ...
    async fn example_put_object_chunked_stream_with_trailers() {
        ...
        let chunk_results: Vec<Result<Bytes, _>> =
            vec![Ok(chunk1), Ok(chunk2), Ok(chunk3), Ok(trailers_block)];
    ...
    async fn unsigned_payload_with_trailer_minimal() {
        ...
        let string_to_sign =
            create_trailer_string_to_sign(&date, region, service, seed_signature, &canonical);
        let sig = sig_v4::calculate_signature(
            &string_to_sign,
            &SecretKey::from(secret_access_key),
            &date,
            region,
            service,
        );
        let trailers_block = Bytes::from(format!(
        ...
        let mut chunked_stream = AwsChunkedStream::new(
            ...
            decoded_content_length,
            true,  // unsigned
            false, // v0.13.0-s4.1 skip_verify
        );
    ...
    async fn unsigned_payload_with_trailer_no_signature() {
        ...
        assert_eq!(
            trailers.get("x-amz-meta-a").unwrap(),
            &HeaderValue::from_static("1")
        );
        assert_eq!(
            trailers.get("x-amz-meta-b").unwrap(),
    ...
    async fn test_signature_mismatch() {
        ...
        assert!(matches!(
            result,
            Err(AwsChunkedStreamError::SignatureMismatch)
        ));
    }
    ...
    async fn test_incomplete_stream() {
        ...
        assert!(matches!(
            result,
            Some(Err(AwsChunkedStreamError::Incomplete))
        ));
    }
    ...
    async fn test_trailer_signature_mismatch() {
        ...
        assert!(matches!(
            result,
            Some(Err(AwsChunkedStreamError::SignatureMismatch))
        ));
    }
    ...
    async fn test_trailer_format_error_invalid_header() {
        ...
        assert!(matches!(
            result,
            Some(Err(AwsChunkedStreamError::FormatError))
        ));
    }
    ...
    async fn test_normal_sized_trailers_work() {
        ...
        let chunk_results: Vec<Result<Bytes, _>> =
            vec![Ok(chunk1), Ok(chunk2), Ok(Bytes::from(trailers))];
    ...
    async fn test_chunk_meta_too_large() {
        ...
        let chunk_results: Vec<Result<Bytes, _>> =
            vec![Ok(Bytes::from(meta_part1)), Ok(Bytes::from(meta_part2))];
        ...
        match result.unwrap() {
            Err(AwsChunkedStreamError::Underlying(e)) => {
                ...
                assert!(
                    matches!(
                        downcasted.unwrap(),
                        AwsChunkedStreamError::ChunkMetaTooLarge(_, _)
                    ),
                    "Expected ChunkMetaTooLarge error",
                );
            }
            other => {
                panic!("Expected ChunkMetaTooLarge error wrapped in Underlying, got: {other:?}")
            }
        }
    }
    ...
    async fn test_trailers_too_large() {
        ...
        for i in 0..400 {
            large_trailers.extend_from_slice(
                format!("x-amz-meta-header-{i}: {}\r\n", "x".repeat(30)).as_bytes(),
            );
        }

        let chunk_results: Vec<Result<Bytes, _>> =
            vec![Ok(chunk1), Ok(chunk2), Ok(Bytes::from(large_trailers))];
    ...
    async fn test_too_many_trailer_headers() {
        ...
        let chunk_results: Vec<Result<Bytes, _>> =
            vec![Ok(chunk1), Ok(chunk2), Ok(Bytes::from(many_trailers))];
