...
pub fn add_opt_header_timestamp<N>(
    res: &mut Response,
    name: N,
    ...
    N: IntoHeaderName,
{
    if let Some(value) = value {
        let val =
            fmt_timestamp(&value, fmt, HeaderValue::from_bytes).map_err(S3Error::internal_error)?;
        res.headers.insert(name, val);
    }
    Ok(())
}
...
pub fn set_xml_body<T: xml::Serialize>(res: &mut Response, val: &T) -> S3Result {
    ...
    res.headers
        .insert(hyper::header::CONTENT_TYPE, APPLICATION_XML);
    Ok(())
}
