...
pub struct S3ServiceBuilder {
    ...
    // v0.13.0-s4.1: when true, SigV4/SigV2 header parsing still runs (so the
    // request canonicalization + streaming body framing keep working) but
    // the final HMAC compare and any per-chunk signature compares are
    // skipped. Lets a trusted-network gateway accept clients whose access
    // key/secret pair isn't pre-registered in the auth provider.
...
impl S3ServiceBuilder {
    ...
    /// v0.13.0-s4.1: opt this service into "NoVerify" mode for trusted-network
    /// deployments.
    ///
    /// When enabled, the service still parses the SigV2 / SigV4 header (or the
    /// presigned query string) so that streaming bodies (`STREAMING-AWS4-HMAC-
    /// SHA256-PAYLOAD` and friends) are decoded and the canonical request is
    /// built — but the final HMAC compare against the client-supplied
    /// signature, and any per-chunk signature compares, are skipped. The
    /// service still calls [`S3Auth::get_secret_key`] so a custom provider
    /// can record the access key; combine with [`SimpleAuth`] for trusted
    /// pre-registered keys, or with [`AcceptAnyAuth`] to accept any access
    /// key. Default `false` keeps the upstream-strict behaviour.
    ///
    /// Designed for an S3-compatibility gateway that sits behind a VPC/SG and
    /// re-signs to the real backend with its own IRSA/instance-role
    /// credentials, where verifying the client signature would force every
    /// caller to use the same one-and-only AKID the gateway happens to have
    /// loaded.
    ///
    /// [`SimpleAuth`]: crate::auth::SimpleAuth
    ...
    pub fn build(self) -> S3Service {
        let config = self
            .config
            .unwrap_or_else(|| Arc::new(StaticConfigProvider::default()));
        S3Service {
            inner: Arc::new(Inner {
                s3: self.s3,
                config,
...
impl S3Service {
    ...
    pub async fn call(&self, req: HttpRequest) -> Result<HttpResponse, HttpError> {
        ...
        let ccx = crate::ops::CallContext {
            ...
            skip_signature_verification: self.inner.skip_signature_verification,
        };
...
mod tests {
    ...
    fn test_service_builder_set_route() {
        ...
        impl S3Route for NoopRoute {
            ...
            async fn call(
                &self,
                _: crate::S3Request<crate::Body>,
            ) -> crate::S3Result<crate::S3Response<crate::Body>> {
