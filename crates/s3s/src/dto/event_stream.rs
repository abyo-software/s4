...
fn request_level_error(e: &S3Error) -> Message {
    ...
    let message = e
        .message()
        .map_or_else(Bytes::new, |s| Bytes::copy_from_slice(s.as_bytes()));
...
mod tests {
    ...
    fn continuation_event_message() {
        ...
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":event-type" && v == "Cont")
        );
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":message-type" && v == "event")
        );
    ...
    fn records_event_with_payload() {
        ...
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":event-type" && v == "Records")
        );
        assert!(
            headers
                .iter()
    ...
    fn progress_event_no_details() {
        ...
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":event-type" && v == "Progress")
        );
        assert!(
            headers
                .iter()
    ...
    fn request_level_error_static_code() {
        ...
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":error-code" && v == "InternalError")
        );
        assert!(
            headers
                .iter()
        ...
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":message-type" && v == "error")
        );
    }
    ...
    fn request_level_error_custom_code() {
        ...
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":error-code" && v == "CustomErr")
        );
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":error-message" && v == "custom message")
        );
    }
    ...
    fn request_level_error_no_message() {
        ...
        assert!(
            headers
                .iter()
                .any(|(n, v)| n == ":error-message" && v.is_empty())
        );
    }
    ...
    fn message_serialize_crc_integrity() {
        ...
        let message_crc_actual =
            u32::from_be_bytes(data[total_len - 4..total_len].try_into().unwrap());
    ...
    async fn wrapper_stream_handles_errors() {
        let events: Vec<S3Result<SelectObjectContentEvent>> =
            vec![Err(S3Error::new(S3ErrorCode::InternalError))];
        let stream = SelectObjectContentEventStream::new(futures::stream::iter(events));
        let mut wrapper = Wrapper(stream);
    ...
    async fn into_byte_stream_produces_bytes() {
        let events: Vec<S3Result<SelectObjectContentEvent>> =
            vec![Ok(SelectObjectContentEvent::End(EndEvent {}))];
        let stream = SelectObjectContentEventStream::new(futures::stream::iter(events));
